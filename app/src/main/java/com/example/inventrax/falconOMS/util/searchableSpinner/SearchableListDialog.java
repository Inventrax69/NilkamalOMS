package com.example.inventrax.falconOMS.util.searchableSpinner;
...
//                _listViewItems.clearTextFilter();
           // ((ArrayAdapter) _listViewItems.getAdapter()).getFilter().filter(null);
            ...
            listAdapter = new ArrayAdapter(getActivity(), android.R.layout.simple_list_item_1,ditems);        //attach the adapter to the list
            _listViewItems.setAdapter(listAdapter);
           // ((ArrayAdapter) _listViewItems.getAdapter()).getFilter().filter(s);
        }
        if (null != _onSearchTextChanged) {
